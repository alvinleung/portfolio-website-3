...
As home cook ourselves, the pain of navigating such digital recipes—video or textural—with dirty hands is all too familiar. Slimy raw meat, dusty flour, there are million of things in the kitchen that makes you think twice before touching your devices. Because of this, we see the oppoutinity to _challenge the dominant touch paradigm_ of modern mobile devices.

## Touchless cooking companion

The solution feature an air-gesture and voice query enabled cooking companion, allowing users to navigate recipes during cooking without ever touching their devices.
...
<LayoutMainContent grid="2/3" extraMargin>
  ...
  >
    <Caption
      label="1"
      text="User can play and pause a video by holding their hands in front of the device."
      overlay={false}
      topPadding
    />
  </Video>
  ...
  >
    <Caption
      label="2"
      text="As user long-hold, the user enters scrubbing mode. They can lean their hands on a side to scrub the video—like using arrow keys."
      overlay={false}
      topPadding
    />
  </Video>
...
<LayoutMainContent>
  ...
  >
    <Caption
      label="3"
      text="Voice query is supported when the for use-cases where the user's hands are occupied by other tasks."
      overlay={false}
      topPadding
    />
  </Video>
...
<LayoutMainContent grid="2/3">
  ...
  >
    <Caption
      label="4"
      text="Follow actions like setting a timer, pinning a card can be access by using voice command right after a response."
      overlay={false}
    />
  </Image>
  <Image
  ...
  >
    <Caption
      label="3"
      text="The measurements mentioned in the response will be highlighted beneath for the ease of access. User can toggle between units by tapping on the pills."
      topPadding
      overlay={false}
    />
  </Image>
...
Other than that, through affinity diragramm we also surfaced the following major
insights:
