...
The project began by surveying the domain of spatial computing and domestic computing. We have found intriguing exploration such as _[on-skin gesture control](https://www.tandfonline.com/doi/full/10.1080/14606925.2022.2058444)_, but that relies on custom hardware. On the consumer market, recipe apps typically focus on pre-cooking phases like recipe management and discovery. Voice enabled solutions are typically very primitive and clunky to navigate.

## Insights from Contextual Inquiries

To probe deeper, we conducted contextual inquiries with 5 home cooks of varying skill levels to observe how they navigate digital recipes. While users are generally able to complete the tasks through workarounds, the operations were less the desirable.
...
While the scrubbing system was easy to understand, it suffer a few drawbacks that makes it particularly tricky to move forward with the direction.
...
This iteration was utilize a circular dialling motion as video scrubbing. The gesture was intentionally referencing the knob-turning motion to appeal to the user's existing mental model.
...
This was chosen to be included in the final solution.
